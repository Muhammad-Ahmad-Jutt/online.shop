...
<<<<<<< HEAD
            ...
            request_count = Verify_Email.objects.filter(user=user, created_at__gte=ten_minutes_ago, used=False).count()
            if request_count>=2:
                raise serializers.ValidationError("User already have two pending request wait for 10 minutes")
            else:
                code = Verify_Email(user=user)
                code.generate_code()
                code.save()
...
<<<<<<< HEAD
            ...
            if request_count>=2:
                ...
                send_mail_to_user(email,message,subject)
                data["status"]=True
        except Exception as e:
